...
describe('nextdate()', function() {
  ...
  it('should work fine when crossing month', function(){
    var d2 = nextdate.nextdate(1999, 6, 30)
    assert.equal(d2.year, 1999);
    assert.equal(d2.month, 7);
    assert.equal(d2.day, 1);
  });

  it('should work fine when crossing year', function(){
    var d2 = nextdate.nextdate(1999, 12, 31)
    assert.equal(d2.year, 2000);
    assert.equal(d2.month, 1);
    assert.equal(d2.day, 1);
  });
});
